<?php
	...
	if ( $_POST['sSearch'] != "" )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
				$sWhere .= $aColumns[$i]." LIKE '%".mysqli_real_escape_string($GLOBALS["___mysqli_ston"],  $_POST['sSearch'] )."%' OR ";
			}
		}
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if ( $_POST['bSearchable_'.$i] == "true" && $_POST['sSearch_'.$i] != '' )
		{
			...
			$sWhere .= $aColumns[$i]." LIKE '%".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['sSearch_'.$i])."%' ";
		}
	}
